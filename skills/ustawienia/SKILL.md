...
**Ta instrukcja pochodzi z wydania 0.22.0.** Numer jest wpisany w tym pliku, więc
...
Interesuje Cię `tag_name` (np. `v0.22.0`). Porównaj z numerem wyżej:
- **te same** → dopisz `Wersja: 0.22.0 (najnowsza)`.
- **wydanie nowsze** → dopisz `Wersja: 0.22.0 — jest już 0.23.0` i powiedz, jak
