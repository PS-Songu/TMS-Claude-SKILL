...
**Ta instrukcja pochodzi z wydania 0.10.0.** Numer jest wpisany w tym pliku, więc
...
Interesuje Cię `tag_name` (np. `v0.10.0`). Porównaj z numerem wyżej:
- **te same** → dopisz `Wersja: 0.10.0 (najnowsza)`.
- **wydanie nowsze** → dopisz `Wersja: 0.10.0 — jest już 0.11.0` i powiedz, jak
